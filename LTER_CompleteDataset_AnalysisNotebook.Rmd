...
```{r lm_richness_BP, fig.cap="Richness (observed OTUs) versus bacterial production during summer months at Palmer Station."}
lm_richness_BP <- lm(richness~Leucine, data=diversitytable)
summary(lm_richness_BP)
plot(richness~Leucine, data=diversitytable)
abline(lm_richness_BP, col="red")
```

Looks like richness~Leucine may be a non-linear relationship.
...
```{r plot_NMDS_all_month, echo=FALSE, fig.cap="NMDS with all samples. Sample shape is coded by LTER season. Sample color is coded by month."}
...
```{r plot_NMDS_all_depth, echo=FALSE, fig.cap="NMDS with all samples. Sample shape is coded by LTER season. Sample color is coded by month."}

coded.otutable_bac.mds$Depth <- as.factor(coded.otutable_bac.mds$Depth)

ggplot(coded.otutable_bac.mds, aes(x=MDS1, y=MDS2, color=Month, shape=Depth)) +
  geom_point(, size=4) +
  scale_color_hue() +
...
### According to daylength

```{r plot_NMDS_all_daylength, echo=FALSE, fig.cap="NMDS with all samples, results color-coded according to day length, i.e. hours between official sunrise and official sunset."}
...
```{r NMDS_PAL0910_and_PAL1011, echo=FALSE, fig.cap="NMDS with only 0-m samples (PAL0910 and PAL1011).", eval=FALSE}
...
```{r NMDS_PAL1112_and_PAL1213, echo=FALSE, fig.cap="NMDS with only 10-m samples (PAL1112 and PAL1213).", eval=FALSE}
...
```{r NMDS_w_ellipses, echo=FALSE, fig.cap="NMDS without PAL0910. Ellipses represent 95% confidence intervals.", eval=FALSE}
#Get MDS stats
